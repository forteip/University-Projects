...
public class MarkDao {
  ...
  public List<String[]> getMarkReport(String courseCode) {
    ...
    PreparedStatement stmt;
    String sql =
        "SELECT DISTINCT userclassmap.utor_id, userassignmarkmap.assignment_id, assignments.assignment_name, userassignmarkmap.mark FROM"
            +
    ...
    try {
      ...
      while (rs.next()) {
        String row[] = {rs.getString("utor_id"), rs.getString("assignment_id"),
            rs.getString("assignment_name"), rs.getString("mark")};
        res.add(row);
      }
      rs.close();
      stmt.close();
  ...
  public List<String[]> getMarkReportForStudent(String utorId, String courseCode) {
    ...
    PreparedStatement stmt;
    String sql =
        "SELECT DISTINCT userassignmarkmap.assignment_id, assignments.assignment_name, userassignmarkmap.mark FROM\n"
            +
            " userassignmarkmap INNER JOIN assignments ON (assignments.id =userassignmarkmap.assignment_id \n"
            +
    ...
    try {
      ...
      while (rs.next()) {
        String row[] = {rs.getString("assignment_id"), rs.getString("assignment_name"),
            rs.getString("mark")};
        res.add(row);
      }
      rs.close();
