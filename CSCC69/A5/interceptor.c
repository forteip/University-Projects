...
void my_exit_group(int status)
{
	...
	//PID is removed from all lists, current variable used
	del_pid(current->pid);
	//Unlock the process
	spin_unlock(&pidlist_lock);
	//Remembering to call the original exit_group
...
asmlinkage long interceptor(struct pt_regs reg) {
	...
	asmlinkage long (*o_sysc)(struct pt_regs);
	spin_lock(&pidlist_lock);
	...
	o_sysc = table[reg.ax].f;
	printk(KERN_DEBUG "End of checks");
	spin_unlock(&pidlist_lock);
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
	...
	if (cmd == REQUEST_SYSCALL_INTERCEPT){
		...
		//check if currently intercepted
        ...
        spin_lock(&calltable_lock);
		//check if not currently intercepted
		...
		//check if this pid is properly owned
		if(current_uid() != 0){
